...
export default function Home() {
  ...
  const [totalScans, setTotalScans] = useState(() => {
    ...
    return stored ? Math.max(parseInt(stored, 10), BASELINE) : BASELINE;
  });
  ...
  useEffect(() => {
    ...
    fetch("/api/scan").then(r => r.json()).then(data => {
      ...
      if (typeof data.totalScans === "number") {
        // Always add baseline on top of real count
        const displayed = BASELINE + data.totalScans;
        setTotalScans(prev => {
      ...
      if (typeof data.hourlyScans === "number") {
        setHourlyScans(data.hourlyScans);
      }
    }).catch(() => {
  ...
  const runScan = async (type: "image" | "url") => {
    ...
    try {
      let res: Response;
      if (type === "url") {
        res = await fetch("/api/scan", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ url: urlInput }) });
      } else {
  ...
  return (
    <main style={{ position: "relative", zIndex: 1 }}>
      ...
      <section style={{ maxWidth: "640px", margin: "0 auto", padding: "48px 24px 36px", textAlign: "center", position: "relative", zIndex: 2 }}>
        <div style={{ fontFamily: "monospace", fontSize: "10px", letterSpacing: "2px", color: "rgba(184,160,232,0.4)", marginBottom: "20px", textTransform: "uppercase" }}>
        ...
        {hourlyScans > 0 && (
          <div style={{ display: "flex", alignItems: "center", justifyContent: "center", gap: "6px", marginBottom: "18px" }}>
            <div style={{ width: "5px", height: "5px", borderRadius: "50%", background: "var(--green)" }} className="animate-pulse" />
            <span style={{ fontSize: "11px", color: "var(--text-3)", fontFamily: "monospace", letterSpacing: "0.3px" }}>
              {hourlyScans} scan{hourlyScans !== 1 ? "s" : ""} in the last hour
            </span>
          </div>
        )}

        <h1 style={{ fontFamily: "'Space Grotesk',sans-serif", fontSize: "clamp(28px,6vw,54px)", fontWeight: "800", lineHeight: "1.08", letterSpacing: "-1.5px", color: "var(--text)", marginBottom: "16px" }}>
        ...
        <p style={{ fontSize: "16px", color: "var(--text-2)", lineHeight: "1.7", maxWidth: "440px", margin: "0 auto 32px" }}>
          Drop any product screenshot or paste a URL. We find the real wholesale cost and show you exactly what you are paying above it.
        </p>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "14px" }}>
          ...
          <span style={{ fontFamily: "monospace", fontSize: "10px", color: "var(--text-3)", letterSpacing: "1px" }}>OR SCAN DIRECTLY</span>
          <div style={{ flex: 1, height: "1px", background: "var(--border)" }} />
        </div>
        ...
        {preview && (
          <div style={{ borderRadius: "14px", overflow: "hidden", marginBottom: "10px", position: "relative" }}>
            ...
            <div style={{ position: "absolute", inset: 0, pointerEvents: "none" }}>
              ...
              {/* Status label */}
              <div style={{ position: "absolute", top: "14px", left: "50%", transform: "translateX(-50%)", fontFamily: "monospace", fontSize: "8px", letterSpacing: "1.5px", color: "rgba(239,68,68,0.8)", background: "rgba(7,7,14,0.7)", padding: "2px 8px", animation: "fadeIn 0.15s ease forwards" }}>TARGET ACQUIRED</div>
            </div>
        ...
        {!preview && (
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "10px" }}>
            ...
            >
              ...
              <div style={{ fontWeight: "700", fontSize: "13px", color: "var(--accent-bright)", marginBottom: "4px", fontFamily: "'Space Grotesk', sans-serif", letterSpacing: "0.5px" }}>
                GALLERY
              </div>
              <div style={{ fontSize: "10px", color: "var(--text-3)", lineHeight: "1.4", fontFamily: "monospace" }}>
            ...
            >
              ...
              <div style={{ fontWeight: "700", fontSize: "13px", color: "#ff8080", marginBottom: "4px", fontFamily: "'Space Grotesk', sans-serif", letterSpacing: "0.5px" }}>
                LIVE SCAN
              </div>
              <div style={{ fontSize: "10px", color: "var(--text-3)", lineHeight: "1.4", fontFamily: "monospace" }}>
      ...
      <section className="reveal" style={{ maxWidth: "640px", margin: "0 auto 48px", padding: "0 24px", position: "relative", zIndex: 2 }}>
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "8px" }}>
          {[
            ...
            { v: "700%", l: "Max markup found" },
            { v: `$${totalSavings >= 1000 ? (totalSavings / 1000).toFixed(1) + "K" : Math.round(totalSavings)}+`, l: "Overcharges exposed" },
          ].map(s => (
      ...
      {/* ═══ CATEGORIES ═══ */}
      <section className="reveal" style={{ maxWidth: "640px", margin: "0 auto 48px", padding: "0 24px", position: "relative", zIndex: 2 }}>
        <div style={{ fontFamily: "monospace", fontSize: "10px", letterSpacing: "2px", color: "rgba(184,160,232,0.4)", marginBottom: "14px", textTransform: "uppercase" }}>WHAT WE CATCH</div>
      ...
      <section className="reveal" style={{ maxWidth: "640px", margin: "0 auto 60px", padding: "0 24px", position: "relative", zIndex: 2 }}>
        <div style={{ borderRadius: "18px", padding: "32px 24px", textAlign: "center", background: "linear-gradient(135deg, rgba(123,94,167,0.1) 0%, rgba(123,94,167,0.03) 100%)", border: "1px solid rgba(123,94,167,0.18)" }}>
          <p style={{ fontFamily: "monospace", fontSize: "10px", letterSpacing: "2px", color: "rgba(184,160,232,0.5)", textTransform: "uppercase", marginBottom: "12px" }}>UNLIMITED ACCESS</p>
          ...
          <p style={{ color: "var(--text-2)", fontSize: "14px", marginBottom: "20px", lineHeight: "1.6", maxWidth: "340px", margin: "0 auto 20px" }}>
            Scan anything. Share the verdict. No limits. No subscription. One payment. Forever.
          </p>
