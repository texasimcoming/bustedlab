...
export async function scanProduct(imageBase64: string, mimeType: string, country?: string, intent?: "verdict" | "finder"): Promise<ScanResult> {
  ...
  // ── Rebrand check: many products sold under a brand name are the same
  //    unbranded item from the original manufacturer with a label added.
  //    If a branded match was found, search once more with the brand name
  //    stripped and the product described only by its generic features.
  //    A genuinely cheaper, visually-verified unbranded match replaces the
  //    branded one — that is the real source, not the relabeled resale. ──
  if (shopping && vision.brand && vision.productName) {
    const unbrandedQuery = `${vision.productName} ${vision.category} ${vision.quantity}`.trim();
    const unbrandedFound = await searchShoppingWithFallbacks([unbrandedQuery, vision.productName]);
    if (unbrandedFound && unbrandedFound.match.lowestPrice > 0) {
      const unbrandedVerified = await verifyCandidates(unbrandedFound.match, imageBase64, mimeType);
      const unbrandedConfident = unbrandedVerified.confidence !== "unverified";
      const currentLow = shopping.lowestPrice > 0 ? shopping.lowestPrice : Infinity;
      if (unbrandedConfident && unbrandedFound.match.lowestPrice < currentLow) {
  ...
  // ── Direct-retailer check: Google's Shopping graph structurally
  //    under-represents several major retailers' own catalogs — a niche
  //    or smaller-brand product frequently shows up there only via the
  //    brand's own paid listing, with a genuinely cheaper Amazon,
  //    Walmart, or eBay listing entirely invisible to that search. Query
  //    those retailers' own search systems directly and, if a genuinely
  //    cheaper verified match turns up, use it. This applies to every
  //    scan regardless of which intent the visitor chose - it happens
  //    before the VERDICT/FINDER branch below, not after it. ──
  if (shopping) {
    const retailerQuery = vision.brand ? `${vision.brand} ${vision.productName}` : (vision.productName || "");
    if (retailerQuery.trim().length >= 3) {
      const retailerFound = await searchAllDirectRetailers(retailerQuery.trim());
      if (retailerFound && retailerFound.match.lowestPrice > 0) {
        const retailerVerified = await verifyCandidates(retailerFound.match, imageBase64, mimeType);
        const retailerConfident = retailerVerified.confidence !== "unverified";
        const currentLow = shopping.lowestPrice > 0 ? shopping.lowestPrice : Infinity;
        if (retailerConfident && retailerFound.match.lowestPrice < currentLow) {
...
export async function scanProductUrl(url: string, country?: string, intent?: "verdict" | "finder"): Promise<ScanResult> {
  ...
  try {
    ...
    if (shopping && pageImage && baseQuery.trim().length >= 3) {
      const retailerFound = await searchAllDirectRetailers(baseQuery.trim());
      if (retailerFound && retailerFound.match.lowestPrice > 0) {
        const retailerVerified = await verifyCandidates(retailerFound.match, pageImage.data, pageImage.mimeType);
        const retailerConfident = retailerVerified.confidence !== "unverified";
        const currentLow = shopping.lowestPrice > 0 ? shopping.lowestPrice : Infinity;
        if (retailerConfident && retailerFound.match.lowestPrice < currentLow) {
          shopping = applyVerifiedCandidate(retailerFound.match, retailerVerified);
