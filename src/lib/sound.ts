...
export function armAudio(): void {
  ...
  try {
    ...
    // iOS Safari in particular sometimes leaves the context reporting
    // "running" after resume() while still not actually producing audio,
    // unless something has genuinely been played inside the gesture. The
    // standard, well-documented unlock for this is playing one real,
    // near-silent buffer synchronously here, not just resuming the
    // context. Gain is effectively inaudible; its only job is to be the
    // first real sound the context produces, inside the click, so the
    // verdict tone eight seconds later is not the first thing it is
    // asked to play.
...
export function playVerdictTone(verdict: VerdictTone): void {
  ...
  const ctx = context;
  // A context can be re-suspended by the browser between the arming click
  // and the tone eight seconds later - a brief tab backgrounding is enough
  // on some browsers. Rather than only checking state and giving up, this
  // waits for a genuine resume before emitting, so a scan that happened to
  // get backgrounded still recovers instead of going silent.
  if (ctx.state === "running") {
    emitVerdictTone(ctx, verdict);
  } else if (ctx.state === "suspended") {
    ctx.resume()
      .then(() => emitVerdictTone(ctx, verdict))
      .catch(() => { /* still blocked; silence stands */ });
  }
}

function emitVerdictTone(ctx: AudioContext, verdict: VerdictTone): void {
